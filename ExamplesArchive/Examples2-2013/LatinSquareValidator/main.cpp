...
// attempt to guess integer square root through trial and error with template metaprogramming
// template< unsigned N, unsigned Guess/* = 1 */>
//  struct ISQRT
//  {
// 	 //if the "guess squared" is less than the original number, guess higher, otherwise stop at that guess
// 	 const static unsigned result = (Guess * Guess < N) ? ISQRT<N,Guess+1>::result : Guess;
//      const static bool isPerfect = (result*result == N);
//  };
//  
// template<unsigned N>
//  struct ISQRT<N,N>
...
//      const static bool isPerfect = false;
//  };
...
public:
    ...
    {
		if(!IntegerSquareRootOf<SQLENGTH>::isPerfect) return false;
        ...
        for(unsigned i=0; i<SQLENGTH; i++)
            for(unsigned j=0; j<SQLENGTH; j++)
            {
                ...
                if(temp <= SQLENGTH && temp > 0)
                {
                    rows[i].insert(temp);
                    cols[j].insert(temp);
					subsqs[i/IntegerSquareRootOf<SQLENGTH>::result + IntegerSquareRootOf<SQLENGTH>::result*(j/IntegerSquareRootOf<SQLENGTH>::result)].insert(temp);
                }
            }
            bool retval = true;
            for(unsigned i=0; i<SQLENGTH; i++)
...
int main()
{
	...
	std::cout << "template meta 4: "<< IntegerSquareRootOf<4>::result << ", isPerfect: " << (IntegerSquareRootOf<4>::isPerfect ? "Perfect" : "imperfect") << std::endl;
	std::cout << "template meta 9: "<< IntegerSquareRootOf<9>::result << ", isPerfect: " << (IntegerSquareRootOf<9>::isPerfect ? "Perfect" : "imperfect") << std::endl;
	std::cout << "template meta 64: "<< IntegerSquareRootOf<64>::result << ", isPerfect: " << (IntegerSquareRootOf<64>::isPerfect ? "Perfect" : "imperfect") << std::endl;
	std::cout << "template meta 1024: "<< IntegerSquareRootOf<1024>::result << ", isPerfect: " << (IntegerSquareRootOf<10>::isPerfect ? "Perfect" : "imperfect") << std::endl;
    system("pause");
}
