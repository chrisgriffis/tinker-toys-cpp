//////////////////////////////////////////////////////////////////////////////
// Product: QBomb Example for Linux
...
struct termios l_saved;         /* structure with saved terminal attributes */
...
int main() {
    ...
    tcgetattr(0, &tio);                 // get the current terminal attributes
    ...
    for (;;) {                                                   // event loop
        ...
        FD_SET(0, &con);
        timeout.tv_sec =  0;
        timeout.tv_usec = 100000;                              // 100 ms delay

        select(1, &con, 0, 0, &timeout);    // wait for the timeout or console
...
void QBomb::exit(void) {
    printf("\nBye, Bye!");
    tcsetattr(0, TCSANOW, &l_saved);  // restore the saved terminal attributes
    ::exit(0);
}
