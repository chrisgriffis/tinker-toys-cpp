...
// Date of the Last Update:  Nov 20, 2005
//
...
class QF_API QEQueue {
private:

    /// \brief pointer to event at the front of the queue
    ///
    /// All incoming and outgoing events pass through the frontEvt_ location.
    /// When the queue is empty (which is most of the time), the extra
    /// frontEvt_ location allows to bypass the ring buffer altogether,
    ...
    /// of the queue. The queue is empty if the frontEvt_ location is NULL.
    QEvent const *frontEvt_;
...
private:
    friend class QF_API QF;
    friend class QF_API QActive;
};
