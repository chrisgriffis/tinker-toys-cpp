...
#define QF_SCHED_UNLOCK()           pthread_mutex_unlock(&QF_pThreadMutex_)
    ...
    /* _GNU_SOURCE give us access to PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP */
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#include <pthread.h>
...
#define QACTIVE_OSOBJECT_WAIT_(me_) \
    if ((me_)->eQueue__.frontEvt__ == (QEvent const *)0) { \
        do { \
            pthread_cond_wait(&(me_)->osObject__, &QF_pThreadMutex_); \
        } while ((me_)->eQueue__.frontEvt__ == (QEvent const *)0); \
    } else ((void)0)

#define QACTIVE_OSOBJECT_SIGNAL_(me_) \
    QF_INT_UNLOCK(); \
    Q_ASSERT((me_)->eQueue__.nTot__ != (QEQueueCtr)0); \
    pthread_cond_signal(&(me_)->osObject__)
...
* and in this port also when QF_tick() is invoked.
*
